...
-- tidal <- startTidal defaultConfig [(target, superDirtShape)]

:{
let target =
      Target {oName = "pd",   -- A friendly name for the target (only used in error messages)
              oAddress = "127.0.0.1", -- The target's network address, normally "localhost"
              oPort = 9003,           -- The network port the target is listening on
              -- ES8 48khz bei 128 Buffer
              -- oLatency = 0.066,
              oLatency = 0.01,
...
:{
let oscmap = [(target, [superdirtShape])]
